...
# integration
set.seed(12345)
integration <- treecor_harmony(count = raw_data[['count']], 
...
```{r,message=F,out.width='\\textwidth', fig.height = 4, fig.align='center'}
# cell clusters
label_text <- integrated_cellmeta %>% group_by(celltype) %>% 
...
df_marker <- t(sub_norm) %>% data.frame %>% 
  mutate(barcode = rownames(.)) %>% gather(gene,expr,-barcode) %>% 
  inner_join(cell_meta %>% select(barcode,UMAP_1,UMAP_2))
...
```{r,warning=FALSE,message=FALSE,out.width='\\textwidth', fig.height = 3, fig.align='center'}
# specify string
...
```{r,warning=FALSE,out.width='\\textwidth', fig.height = 6, fig.align='center'}
# visualize
...
```{r,message=FALSE,warning=FALSE,fig.width=5, fig.height = 5, fig.align='left'}
...
```{r, message=F,warning=F,out.width='\\textwidth', fig.height = 6, fig.align='center'}
# aggregate
colnames(res_expr) <- gsub('severity','Severity\n(Aggregate)',colnames(res_expr))
# concat_leaf
colnames(res_expr_concatLeaf) <- gsub('severity','Severity\n(ConcatLeaf)',colnames(res_expr_concatLeaf))
# concat_immediate_children
colnames(res_expr_concatImmChild) <- gsub('severity','Severity\n(ConcatImmChild)',colnames(res_expr_concatImmChild))
